...
*By [Your Name] — [month] 2026*

I have been a software engineer for 27 years. In the past year the way I work has changed more than in the previous ten combined. I barely write code anymore. I still do all of the engineering.

That distinction is what this essay is about.

## How I actually work now

My days look like this. I write plans. Before any code exists, I spend real time — sometimes hours, sometimes a day — writing down the problem, the constraints, the failure modes, the design decisions and the reasoning behind them, the alternatives I considered and rejected. I iterate on that plan, and increasingly I use an LLM to challenge it before a human ever sees it.
...
That is the part I find genuinely underserved, in both the discourse and the tooling. More on the tooling at the end, because I owe you a disclosure there.

## A story from production

> **[WAR STORY — replace this entire block before publishing. The essay does not work without it.]**
>
> The scaffold: pick one real incident from the last year where the plan-first workflow earned its keep, or — even better — where the model's fluent output was subtly wrong and your review caught it. Anonymise the domain details as far as you need to. The shape that works:
>
> 1. What you were building, in one or two sentences a non-specialist can follow.
> 2. What the model produced, and why it looked right. Be specific about the seduction — clean structure, passing tests, sensible names.
> 3. The moment of interrogation: what looked slightly off, what you asked, what the answer revealed.
> 4. What it would have cost if you had accepted the output. Be honest if the answer is "an annoying afternoon" rather than "a disaster" — honesty here buys credibility for the whole essay.
> 5. Where the constraint or reasoning now lives, and who else can find it.
>
> Include one messy detail that a morality tale would leave out. That detail is what makes it believable.

## Why this matters more than it looks: theory building

In 1985 Peter Naur argued that a program is not its source code. The real artifact of programming is the theory — the mental model, held by the people who built it, of how the system works, why it is shaped the way it is, and how it can change without coming apart. Source code is a lossy representation of that theory. When the people who hold the theory leave, the program effectively dies, whatever the documentation says.
...
**This is not universal.** Everything above assumes systems where being subtly wrong is expensive: systems that handle money, safety, scale, or long lifetimes. That describes my work and a lot of professional software engineering. It does not describe all of it. For a genuinely large share of software — internal tools, prototypes, short-lived features — fast, shallowly-understood code plus cheap regeneration may be the economically correct choice. I think the boundary between those regimes is one of the most important judgment calls an engineering leader now makes, and most are making it by default rather than on purpose.
...
Plans, not tickets, should be the unit at which engineering work is coordinated, reviewed, and evaluated. Status should be derived from the artifacts of work, not manually reported alongside it; if engineers maintain two representations of reality, one of them will drift. Review effort should be proportional to reasoning risk, not diff size. And hiring should weight how someone reasons about a system under uncertainty — and how their judgment holds up when the model is confidently wrong — over how fast they produce code, because that second question has stopped discriminating.

## A disclosure, and an invitation

The ideas here came out of my own frustration, and that frustration has a practical edge: I intend to build tooling to support this way of working — in particular, collaborative planning as a first-class object, with status derived from it rather than reported around it. I am telling you this now because I would rather you read the essay knowing it. The ideas came first, and they stand or fall independently of anything I build.
...
*[Your Name] is a software engineer. Opinions here are personal.*
