...
**Verification is not free.** Interrogating every consequential decision a model makes costs attention, and reading code critically has always been harder than writing it. If model output keeps accelerating, the Critical Engineer risks becoming a choke point in front of an arbitrarily fast generator. I don't have a complete answer. In practice I triage: the depth of interrogation scales with the blast radius of the decision, and I lean on the model itself to generate the adversarial tests I would once have written by hand. Whether that scales for a whole team is an open question, and I would distrust anyone who claims to have settled it. A promising answer may not be a process but a technology: world models, systems that learn how a piece of software behaves well enough that "what if" becomes something you run rather than argue about. If that matures, checking stops being one person reading everything in sequence and becomes something you can run wide. I hold that loosely. Nearly all the work so far is in robotics and self-driving, not business systems, and long chains of what-if drift away from reality.

**Better models will eat some of this.** Some of the judgement I describe, spotting the unsafe concurrency assumption, noticing the unencoded business constraint, will increasingly be within model reach. What I don't believe gets automated is accountability. When a system fails at scale, someone is responsible, and that someone cannot be a model. But I will concede that accountability without genuine epistemic contribution is a thin thing, and if verification hollows out entirely, this essay's model hollows out with it. And theory may be more exposed than I first allowed. Naur says theory cannot be written down, and he is right about documentation, because documentation cannot be wrong in a way anyone notices. A model that makes predictions and gets corrected by what actually happens is a different proposition. But theory has two halves: what the system does, and what it is for. A world model can learn the first and not the second. A user entering the contract end date in a notes field is behaviour a model reproduces faithfully without ever knowing it is a workaround; intent is not in the data. So if this plays out, the Critical Engineer's job narrows toward intent and away from mechanism. Not a smaller job, a better-specified one. My working test for any claim in here: if AI became significantly more capable, would this still be true? The claims about coordination and accountability pass. Theory passes in halves: the descriptive half may be delegated the way expression was; the intentional half I cannot see how to delegate. Some claims about day-to-day review mechanics may not, and I expect to revise them.
